...
//!
//! Each test starts `noyalib-mcp --transport ... --port 0`, reads the
...
impl Http {
    fn send(addr: &str, method: &str, path: &str, headers: &[(&str, &str)], body: &str) -> Self {
    ...
    fn fill(&mut self) -> bool {
        if self.chunked {
            ...
            let size = usize::from_str_radix(size.trim(), 16).expect("hex size");
            if size == 0 {
                return false;
            }
...
fn streamable_http_holds_a_session_with_a_handshake() {
    ...
    assert_eq!(init["result"]["serverInfo"]["name"], "noyalib-mcp");
    drop(r);
...
fn streamable_http_serves_stateless_requests_without_a_session() {
    ...
    let mut r = Http::send(
        ...
        &stateless(
            3,
            "tools/call",
            json!({"name": "noyalib_parse", "arguments": {"yaml": "a: 1\n"}}),
        ),
    );
    assert_eq!(r.status, 200);
    let call = r.next_message();
    assert_eq!(call["result"]["isError"], false, "{call}");
    assert_eq!(call["result"]["structuredContent"]["documents"][0]["a"], 1);
    drop(r);
    ...
    let mut r = Http::send(
        ...
        &stateless(
            4,
            "tools/call",
            json!({"name": "noyalib_parse", "arguments": {"yaml": "a: [\n"}}),
        ),
    );
    let call = r.next_message();
...
fn sse_opens_a_stream_and_answers_posts_on_it() {
    ...
    let r = Http::send(
        ...
        &endpoint,
        &[JSON],
        r#"{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"noyalib_edit","arguments":{"yaml":"a: 1\n","path":"a","value":"2"}}}"#,
    );
    assert_eq!(r.status, 202);
    let call = stream.next_message();
    assert_eq!(call["result"]["content"][0]["text"], "a: 2\n", "{call}");
...
fn a_port_in_use_is_reported_not_swallowed() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_noyalib-mcp"))
