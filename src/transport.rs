    ...
    while let Some(arg) = args.next() {
        ...
        match flag.as_str() {
            ...
            "--transport" => {
                ...
                options.transport = Transport::parse(&name).ok_or_else(|| {
                    format!(
...
async fn open_stream<H: ServerHandler>(State(state): State<Arc<SseState<H>>>) -> Response {
    ...
    let messages = outbox.map(|message| {
        serde_json::to_string(&message).map(|json| Event::default().event("message").data(json))
    });
...
struct SessionStream {
    events: Pin<Box<dyn Stream<Item = Result<Event, serde_json::Error>> + Send>>,
